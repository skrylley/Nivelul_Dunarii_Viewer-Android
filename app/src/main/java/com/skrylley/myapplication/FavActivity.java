...
public class FavActivity extends AppCompatActivity {
    boolean vectorFav[];/*[] = { true, false,  true, false, false,
                            false, false, false, false, false,
                            false, true,  false, false, false,
                            false, false, false, false, false,
                            false, false, true  };*/
    int contorVectorFav = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(global.getGlobalVariableColorCheck())
        {
            ...
            for (int i = 0; i < contorVectorFav; i++) {
                /*if(vectorFav[i] == 1)*/ {
    ...
    public void updateLocalitate(String[] val) {
        ...
        int k=0;
            for (int i = 0; i < contorVectorFav; i++) {
                ...
                if (valLoc[i] != null) {
                    while(vectorFav[k] != true)
                        k++;
                    textView.setText(valLoc[k]);;                  // 0 2 11 22
                    //Log.d("Parse", "FINAL: " + (k));    // 0 2  5  6
                } else
